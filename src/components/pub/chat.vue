    ...
    <div class="chatWindow">
      ...
      <div class="chat">
          ...
          <div class="user" :class="{select:isSelected[index],hovers:isHovered[index]}"
            ...
            <div class="down">
              <a-badge :count="redPoint[index]" size="small" style="float: left;">
                <user :user="chat.user">
                  <a-avatar :class="{online:chat.user.userStatus}" :src="p(chat.user.userProfilePhoto)" size="large"/>
                </user>
              </a-badge>

              <div class="right">
                <div id="uname">
                  <router-link :to="'/user/'+chat.user.userId"> {{ chat.user.userNickname }}</router-link>
                </div>
        ...
        <div class="chatMessage" v-loading="$loading">
          <div class="userIntro">
            <div class="uname">
              <router-link :to="'/user/'+talkTo.user.userId"> {{ talkTo.user.userNickname }}</router-link>
            </div>
          ...
          <div class="chatting" ref="chat" id="chat">
            <template v-for="(record,index) in talkTo.record" :key="index">
              <div id="updateTime" v-if="timeDiff(index,index-1)">{{ this.$moments(record.recordUpdateTime) }}</div>
              <template v-if="record.userId != talkTo.user.userId">
                ...
                <a-dropdown :trigger="['hover']">
                  ...
                  <template #overlay>
                    <a-menu>
                      <a-menu-item key="1" @click="deleteRecord(record.recordId)">删除</a-menu-item>
                      <a-menu-item key="3" @click="deleteRecord(record.recordId)">撤回</a-menu-item>
                    </a-menu>
                  </template>
                </a-dropdown>
